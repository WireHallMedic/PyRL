...
class UnboundScreenObj(screen_obj.ScreenObj):
   ...
   def __init__(self, index = ' ', fg = ColorConstants.WHITE, bg = None):
      ...
      Initializer
      """
      super().__init__(index, fg, bg)
      self.loc_tiles = [-1, -1]
      self.offset = [0.0, 0.0]
   
   def blit_to_surface(self, surface, palette)
      """
      Calculates position and blits to passed surface   
      """
      x_loc = int(self.loc_tiles[0] + self.offset[0] * palette.tile_size_px[0])
      y_loc = int(self.loc_tiles[1] + self.offset[1] * palette.tile_size_px[1])
      surface.blit( (x_loc, y_loc))